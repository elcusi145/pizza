 ...
 * @author Gabe50
 */
public class Orden_cadetes extends javax.swing.JFrame implements Runnable{
    ...
    public Orden_cadetes(Usuario user) {
        initComponents();
        h1= new Thread(this);
        h1.start();
    ...
    public void pedidos() throws DataAccessException{
        ...
        CadeteDAO ped = new CadeteDAO();

        cadetes = ped.ordenCadete();
    ...
    public void agregar(){
        ...
        if (jTable1.getValueAt(jTable1.getSelectedRow(),0)==null){
            ...
            if(mochila.length==0){
                ...
                primero[0][0] = cadetes[i][0];
                primero[0][1] = cadetes[i][1];
                primero[0][2] = cadetes[i][2];
                ...
                primero[pos][0] = cadetes[i][0];
                primero[pos][1] = cadetes[i][1];
                primero[pos][2] = cadetes[i][2];
                mochila = new Object[pos+1][4];
                mochila=primero;
            }
            
            Object[][] menor = new Object[cadetes.length-1][3];
            int k=0;
            for (int j = 0; j < cadetes.length; j++) {
                if(j!=i){
                    menor[k][0] = cadetes[j][0];
                    menor[k][1] = cadetes[j][1];
                    menor[k][2] = cadetes[j][2];
                    k++;
                }
            }
            cadetes = new Object[menor.length][3];
            cadetes = menor;
            ordenar();
            iniciarListado();
            iniciarMochila();
    ...
    public void quitar(){
        ...
        if (jTable2.getValueAt(jTable2.getSelectedRow(),0)==null){
            ...
            if(cadetes.length==0){
                ...
                cadetes=new Object[1][3];
                cadetes=primero;
            }else{
                int pos = cadetes.length;
                Object[][] primero = new Object[pos+1][4];
                for (int j = 0; j < cadetes.length; j++) {
                    primero[j][0] = cadetes[j][0];
                    primero[j][1] = cadetes[j][1];
                    primero[j][2] = cadetes[j][2];
                }
                primero[pos][0] = mochila[i][0];
                primero[pos][1] = mochila[i][1];
                primero[pos][2] = mochila[i][2];
                primero[pos][3] = mochila[i][3];
                cadetes = new Object[pos+1][3];
                cadetes =primero;
            }
    ...
    public void setOrden() throws DataAccessException{
        if(mochila.length!=0){
            CadeteDAO cadete = new CadeteDAO();
            for(int i=0; i<mochila.length;i++){
                int cod = Integer.parseInt(mochila[i][0].toString());   
                int orden = Integer.parseInt(mochila[i][3].toString());
                cadete.actualizarOrden(cod, orden);
            }
        } 
    }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        try {
            ...
            Logger.getLogger(Orden_cadetes.class.getName()).log(Level.SEVERE, null, ex);
        }
       //dispose();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Orden_cadetes.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
